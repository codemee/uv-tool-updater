...
> 狀態：早期 MVP（`0.1.9`）。在 1.0 之前，公開 API 與結果結構仍可能調整。
...
`show-version` 會顯示目前版本、查詢 PyPI，並在有新版時詢問是否更新。只有輸入 `y` 或 `Y` 才會啟動外部 helper。原程式結束後，helper 會以已驗證的目標版本執行 `uv tool upgrade uv-tool-updater==<版本>`。主控台命令會在沒有互動終端的背景重新啟動；請再次執行 `show-version` 以顯示保存的結果。
...
此參數只替換版本檢查資料；實際更新仍使用 uv 記錄的安裝來源。由背景 helper 重新啟動的主控台命令沒有互動終端，因此不會消費結果；請在原本的終端再執行一次 `show-version`。GUI 與系統匣應用程式仍會正常重新啟動。
